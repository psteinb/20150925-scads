...
def random_enhance_color(_image, _verbose=False, _seed = 0, _color_id = -1):
    ...
    if _verbose:
        print "enhancing channel %i by %i ticks" %(_color_id,delta)
...
def change_and_write(_fname,_image,_args):
    ...
    if not _args.rotate_only:    
        value = random_enhance_color(value, _args.verbose)
